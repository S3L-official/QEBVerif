...
def propagate_difference(in_layer, out_layer, w, b, w_cont, b_cont, layer_index):
    ...
    if not out_layer.signed_output:
        min_val, max_val = qu.uint_get_min_max_integer(
            out_layer.quantization_config["quantization_bits"],
            out_layer.quantization_config["quantization_bits"]
            - out_layer.quantization_config["int_bits_activation"],
...
class LayerEncoding_gurobi:
    def __init__(
        ...
        self.diff_algebra_lower = []  # list of list

        self.diff_algebra_upper = []  # list of list
...
class QNNEncoding_gurobi:
    ...
    def sat(self, args, prediction):
        ...
        errorString = str(error).strip('.')

        fo = open(path + "/" + str(args.arch) + "_qu_" + str(args.qu_bit) + "_" + str(sample_id) + "_attack_" + str(
            eps) + "_err_" + errorString + "_mode_" + str(mode) + "_ifDiff_" + str(ifDiff) + "_gp.txt", "w")
        ...
        fo.write(vadi + "\n")
        fo.write("DRA Time: " + str(self._stats["IP_time"]) + "\n")
...
def check_robustness_gurobi(encoding, x, args, original_prediction):
    eps = args.eps
    path = args.outputPath
    sample_id = args.sample_id
    ...
    if abs(target_diff_lb) <= args.error and abs(target_diff_ub) <= args.error:
        ...
        fo = open(
            path + "/" + str(args.arch) + "_qu_" + str(args.qu_bit) + "_" + str(args.sample_id) + "_attack_" + str(
                args.eps) + "_err_" + str(
                args.error) + "_mode_" + str(args.mode) + "_ifDiff_" + str(args.ifDiff) + "_gp.txt", "w")
    ...
    encoding.error_bound_gurobi_absoluateValue(args.error, args.mode, original_prediction)
