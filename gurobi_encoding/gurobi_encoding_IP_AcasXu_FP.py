...
def propagate_difference(in_layer, out_layer, w, b, w_cont, b_cont, layer_index):
    ...
    if not out_layer.signed_output:  # output layer
        ...
        for out_index in range(out_layer.layer_size):
            ...
            lb_diff_in_1, ub_diff_in_1 = compute_lb_ub(weight_diff, in_layer.clipped_lb_cont,
                                                       in_layer.clipped_ub_cont,
                                                       bias_diff)
        ...
        for i, l in enumerate(self.dense_layers):
            if i == len(self.dense_layers) - 1:
                ...
                for out_index in range(l.layer_size):
                    ...
                    lb_cont_clipped = np.float32(self.deepPolyNets_DNN.layers[2 * (i + 1)].neurons[
                                                     out_index].concrete_lower)
                    ub_cont_clipped = np.float32(self.deepPolyNets_DNN.layers[2 * (i + 1)].neurons[
...
def check_robustness_gurobi_DRA_only(encoding, x_lb, x_ub, args, original_prediction):
    ...
    if abs(target_diff_lb) <= args.error and abs(target_diff_ub) <= args.error:
        print("\nWe directly verified and the error bound property hold!!!")
        fo = open(
            path + "/FP_" + str(args.property) + "_Net_" + str(args.AcasNet) + "_qu_" + str(
                args.qu_bit) + "_eps_" + epsString + "_mode_" + str(args.mode) + "_DRA.txt", "w")
        ...
        print("\nWe can not directly verified via DRA!!!")
        fo = open(
            path + "/FP_" + str(args.property) + "_Net_" + str(args.AcasNet) + "_qu_" + str(
                args.qu_bit) + "_eps_" + epsString + "_mode_" + str(args.mode) + "_DRA.txt", "w")
